import {
  ...
  CITY_AUTOCOMPLETE_LIMIT,
  CITY_GEOCODING_API,
  CITY_GEOCODING_GET_API,
  DEFAULT_SETTINGS,
  DEFAULT_CITY,
} from "./config.js";
...
async function locateCityByGeolocation() {
  ...
  if (!isGeolocationSecureContext()) {
    setBadge("error", "HTTPS required");
    window.alert("Геолокация доступна только по HTTPS (или на localhost).");
    return;
  }
...
function getGeolocationErrorMessage(error) {
  ...
  if (error instanceof Error) {
    ...
    if (text.includes("secure") || text.includes("only secure")) {
      return "Геолокация доступна только по HTTPS (или на localhost).";
    }
  }
...
function renderSnapshot(snapshot) {
  ...
  renderHourlyForecast(snapshot.forecast || null);
  renderWeeklyForecast(snapshot.forecast || null);
}

function renderHourlyForecast(forecast) {
  if (!els.hourlyForecastGrid) {
    return;
  }

  const providerLabel = getForecastProviderLabel(forecast && forecast.provider);
  if (els.hourlyForecastProvider) {
    els.hourlyForecastProvider.textContent = `Почасовой: ${providerLabel}`;
  }

  if (els.hourlyForecastSummary) {
    const base = "Следующие 24 часа: температура и состояние";
    els.hourlyForecastSummary.textContent =
      forecast && forecast.generatedAt ? `${base} | обновлено ${formatClock(forecast.generatedAt)}` : base;
  }

  const hourly = forecast && Array.isArray(forecast.hourly) ? forecast.hourly : [];
  const upcoming = selectUpcomingHours(hourly, 24);
  if (!upcoming.length) {
    els.hourlyForecastGrid.innerHTML = `
      <article class="hourly-forecast-item">
        <p class="hourly-forecast-time">--:--</p>
        <p class="hourly-forecast-temp">--</p>
        <p class="hourly-forecast-condition">Почасовой прогноз недоступен</p>
        <p class="hourly-forecast-extra">Данные появятся после обновления</p>
      </article>
    `;
    return;
  }

  els.hourlyForecastGrid.innerHTML = upcoming.map((row, index) => renderHourlyForecastItem(row, index)).join("");
}

function renderHourlyForecastItem(row, index) {
  const condition = translateCondition(row.condition) || "--";
  const chancePct = Number.isFinite(row.precipChancePct) ? `${Math.round(row.precipChancePct)}%` : "--";
  const precipMm = Number.isFinite(row.precipMm) ? `${numberRu(round1(row.precipMm))} мм` : "--";

  return `
    <article class="hourly-forecast-item">
      <p class="hourly-forecast-time">${escapeHtml(formatHourlySlotLabel(row.parsedTime, index))}</p>
      <p class="hourly-forecast-date">${escapeHtml(formatHourlyDateLabel(row.parsedTime))}</p>
      <p class="hourly-forecast-temp">${formatCompactTemperature(row.tempC)}</p>
      <p class="hourly-forecast-condition">${escapeHtml(condition)}</p>
      <p class="hourly-forecast-extra">${chancePct} | ${precipMm}</p>
    </article>
  `;
}

function selectUpcomingHours(rows, limit) {
  ...
  const normalized = rows
    .map((row) => {
      const parsedTime = parseHourlyForecastDate(row);
      if (!parsedTime) {
        return null;
      }
      return {
  ...
  let startIndex = normalized.findIndex((row) => row.parsedTime.getTime() >= nowMs - graceMs);
  if (startIndex === -1) {
    startIndex = Math.max(0, normalized.length - limit);
  }

  return normalized.slice(startIndex, startIndex + limit);
}

function parseHourlyForecastDate(row) {
  if (!row || typeof row !== "object") {
    return null;
  }

  const fromTime = parseForecastDate(row.time);
  if (fromTime) {
    return fromTime;
  }

  if (/^\d{4}-\d{2}-\d{2}$/.test(String(row.date || "")) && Number.isFinite(row.hour)) {
    const hour = String(clamp(Math.round(row.hour), 0, 23)).padStart(2, "0");
    return parseForecastDate(`${row.date}T${hour}:00:00`);
  }

  return null;
}

function formatHourlySlotLabel(date, index) {
...
function formatHourlyDateLabel(date) {
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) {
    return "--";
  }
  return new Intl.DateTimeFormat("ru-RU", {
